...
!
     ...
     &                            'last  ', Tmax_code, F_code
              END IF
              exit_flag=4
              IF (FoundError(exit_flag, NoError, 296,                   &
     &                       "ROMS/Utility/check_multifile.F")) RETURN
            END IF
          END IF
        END IF
...
!
     ...
     &                              Tmax_code, F_code
                END IF
                exit_flag=4
                IF (FoundError(exit_flag, NoError, 499,                 &
     &                         "ROMS/Utility/check_multifile.F")) RETURN
              END IF
            END IF
          END IF
...
!
      ...
      IF (FoundError(exit_flag, NoError, 595,                           &
...
!
     ...
     &                     ncid = ncid)
      IF (FoundError(exit_flag, NoError, 605,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
...
!
     ...
     &                      start = (/1/),                              &
     &                      total = (/1/))
      IF (FoundError(exit_flag, NoError, 688,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
      CALL netcdf_get_time (ng, model, ncname, TvarName,                &
     ...
     &                      start = (/Nrec/),                           &
     &                      total = (/1/))
      IF (FoundError(exit_flag, NoError, 696,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
!  Close NetCDF file.
!
      CALL netcdf_close (ng, model, ncid, ncname, .FALSE.)
      IF (FoundError(exit_flag, NoError, 702,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
